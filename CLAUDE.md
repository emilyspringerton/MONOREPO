...
| `WOTAN` | Static HTML/CSS | New repo (2026-09-03), upstream pre-created by the founder empty (kanban `WOTAN-REPO-001`, "GITHUB REPO CREATED") — same pattern as `LO`/`SPIDERBEETLE`/`EMILY_FOR_BUSINESS`. Splits WOTAN, EINHORN_INDUSTRIAL's esports/stats hub, out of `OKEMILY/tournaments.html` (a single page in the marketing site) into its own repo/subdomain, since real feature work is now scoped against it: a BRAWLPIT cosmetic hat store paid in GFD Flow with a real pixel editor (`BRAWLPIT/docs/WOTAN_HAT_STORE_NORTHSTAR.md`) and its own DNS (`WOTAN-DNS-001`, real Cloudflare key at `EMILY/var/cloudflare.md`, not yet wired). Real first commit: a placeholder `index.html` matching `OKEMILY/tournaments.html`'s own design system (same `:root` palette), `CLAUDE.md`, `README.md`. Not deployed — `OKEMILY/tournaments.html` stays the live page until this repo actually replaces it. |
| `DEADWEIGHT` | C + Kotlin/Java (planned), PARENA mods | New repo, upstream pre-created by the founder with one real source doc pulled in 2026-09-11: a captured Gemini transcript (misleadingly named `LeetCode Skills Course Curriculum.pdf`) that spirals from an actual LeetCode teaching curriculum into a full PvP game spec, "Dark Sector: Hold Battles" — a 6x6-grid spatial-knapsack real-time combat game (splittable items with a real fragmentation tax, energy routing from generators into perimeter weapons/armor). Real critical review, not rubber-stamped (`DEADWEIGHT/NORTHSTAR.md`, golden doc `DEADWEIGHT-NORTH`): names the transcript's own escalation into a multi-year live-service catalog (a Black-Scholes options-pricing engine, Merkle-tree cargo proofs, 24 items, 16 Ultimates × 3 Flow tiers, a double-elimination tournament bracket, 2v2 team battles) and proposes a real, buildable V0 cut instead (core grid+combat, 6-9 items, 2-3 Ultimates, a simple 1v1 ladder — the rest named as real, deferred future phases, not silently dropped). Real capability audit against the founder's own "build it with PARENA, target Android+Windows, 1 client like Hearthstone" framing: PARENA's C emitter is mature but this monorepo's own universal precedent (REDGARDEN/ECOWAR/PAPERCRAFT) is a hand-written C host + PARENA mod-logic islands, never a whole client emitted from PARENA; PARENA's own Java emitter is real but v0/scalar-only (no structs/loops/collections — its only precedent, `SPIDERBEETLE`, is two standalone helper functions), so "1 PARENA client, 2 platforms" isn't real yet — recommends two native shells (C/SDL2 Windows, Kotlin/Java Android per `MJOLNIR`'s own template) sharing one server-authoritative backend, plus (per the founder's own "meta ui framework" reference) `EOSUI-NORTH`'s real, not-yet-built Option C PARENA-native styling layer as the actual cross-platform design-language unification point. Names guest-account auth (name-only, no email, unrecoverable without linking one — the mobile-game convention the founder asked for) as genuinely new IDUNA work, checked directly: IDUNA's `players` table supports only `google`/`iduna_local` providers today, no anonymous provider exists anywhere in this codebase. Plans to reuse ECOWAR's real matchmaker/bot-pool pattern and REDGARDEN's real IDUNA player/match-tracking model directly rather than reinventing either. No code written yet, see `NORTHSTAR.md`'s own 6-phase plan. |
| `BIG_O` | C/SDL2 (SHANKPIT engine) + PARENA rules | New repo (2026-09-18), NORTHSTAR only, local repo (no upstream yet — founder creates it) — "A SHANKPIT Story": hard sci-fi social-stealth management sandbox. Day: harvest genetic data from feral vectors (SHANKPIT FPS); night: blend into a paranoid corporate night-society (Attention/Heat system, "if they don't see it, it isn't real"; canceled = fail state); basement: off-books cloning lab + pheromone-commanded vector army in an async deterministic shadow war. Checked capability audit vs SHANKPIT (level chain/doors/REFLUX/NOCK characters real; humanness NPCs/witness primitive/lab/swarm/war sim not built). Recommended multiplayer-first (shared persistent world, co-op day, async 1v1 shadow war + bots, DEADWEIGHT server/league pattern). V0 = one day-night-lab turn, 2 players. See `BIG_O/NORTHSTAR.md`. |
| `OpenExecutive` | Python (FastAPI) + Next.js 15 | Forked 2026-09-20 from [SenteLabsAI/OpenExecutive](https://github.com/SenteLabsAI/OpenExecutive) (real, credited prior art, Apache 2.0, 155+ merged upstream PRs of history) — a multi-agent "virtual corporate executive" (CSO/CFO/CHRO/GC/COO/CMO/CPO/Board specialists behind one coherent persona), Anthropic Claude backbone, ChromaDB RAG, SQLite episodic memory, Slack/email/Telegram/Discord/Google Chat/MCP integrations. IDUNA identity (M2M provisioning, `POST /api/v1/openexecutive/provision`, real and live) + Google Gemini backbone (via the Gemini Developer API — a plain key, no GCP Console work — or Vertex AI) both real and live as of S506 (2026-09-21): a real chat request through the full stack (real IDUNA JWT → `IDUNAJWTValidator` → orchestrator → real specialist tools → Gemini) returned a real, correct response end to end for the first time. Deployed live at `https://exec.okemily.com` (the real product UI, its own Google-OAuth-gated login — deliberately separate from IDUNA's session, since this app is designed to also run standalone) plus an IDUNA Back Office admin/provisioning page (`/admin/openexecutive`). See `OpenExecutive/NORTHSTAR.md` §6–8 for the full account of what was found and fixed (an authorization gap, a JWKS staleness regression, three more real bugs found only by actually running a live turn end to end). |
| `GOLDEN_DOCS` | Markdown (generated) | New repo (2026-09-21) — a full, real, git-tracked snapshot of every golden doc listed in `EMILY/context/golden-docs-index.md` (234 docs as of first push) plus core EMILY context files (`full-system-context.md`, the sprawl memo, `golden-repos.json`, this file, `EMILY/docs/ECOSYSTEM_CONTINUITY_REPORT.md`), each hashed in its own `MANIFEST.md`. No source of its own — every file is a plain copy, mirroring its real path in this monorepo. Kept in sync manually for now (see "GOLDEN_DOCS Sync" below and `GOLDEN_DOCS/CLAUDE.md`'s own regeneration script) rather than via CI, a deliberate token-conscious choice pending real automation — tracked in `EMILY/BACKLOG.md`. |
| `SLOWBOT_LEAGUE` | Go/C (planned) | New repo (2026-09-22), NORTHSTAR only, local repo (no upstream yet — founder creates it) — an Elo/PFSP "slow bot league" spin-off of SHANKPIT: humans play against, and train, a persistent PFSP bot pool seeded from SHANKPIT's own live active training checkpoint (gen-0 = checkpoint 578, Elo 1564, confirmed live via `GET /api/v1/shankpit-checkpoints/active`), lives-based ("last player standing" per round) rather than continuous-respawn, on its own server process separate from the live queue. Real capability audit found most of the "totally separate IDUNA tenant" ask already exists and is generic: `IDUNA/internal/games/games.go`'s `Registry` (one new row + a migration, not a new subsystem), guest-auth/Claim Account (`game_online.go`, already `game`-parameterized), and `SHANKPIT/scripts/rl_league.py`'s own game-agnostic PFSP+Elo math (ported verbatim from BRAWLPIT already). Real, genuinely new work named: a lives-based native SHANKPIT game mode (`MODE_QUEUE` today is continuous-respawn FFA, no elimination mode exists), a second server process, and a WOTAN leaderboard-section pattern that doesn't exist yet for ANY game including DEADWEIGHT (built once in Phase 3, reused twice). One real, named ambiguity in the founder's own lives-assignment spec (skill-handicap direction vs. fixed slot order) left open rather than guessed at. See `SLOWBOT_LEAGUE/NORTHSTAR.md`'s own phased plan and three open questions. |
| `IDUNA.GAME` | Go (SDL2, forked from PITVIPER) | New repo (2026-09-22) — founder real-time, redirected mid-build twice ("we need to build an IDUNA SHANKPIT_OS app... open pages in the IDUNA brand affordances" → "not a browser a true port of iduna affordances into sdl 2" → "have it be a reskin of pitviper into solarized light with a text based prompt system" → "build in REFLUX"): a real fork of `PITVIPER` (its own real engine — PTY/SSH/MUD, vterm, GPU glyph atlas — kept exactly as-is, same "new fork, not rewrite-in-place" precedent `SAND` already set against PITVIPER), reskinned to real, canonical Solarized Light, with a text-based prompt (`cmd/idunagame/iduna_prompt.go`) that shows IDUNA's real honor code (bundled verbatim from `IDUNA/internal/honorcode/honorcode.go`) and walks the user through IDUNA's real device-auth login flow (`/auth/device/start`→poll→exchange) against a live IDUNA instance — never embedded Google OAuth directly (a real platform constraint, not a gap). Found and documented, not worked around: a real, confirmed ES256-vs-HS256 token-namespace mismatch between the web ceremony's core-IAM endpoints and the device-auth bridge's own kikoryu-scoped exchange token. New `internal/reflux/` — a native Go port of `SHANKPIT/packages/reflux/reflux_runtime.c`'s own pub/sub ring-buffer (same API/ABI), dispatching real milestones with no subscriber yet, by design. Live-verified via a real headless (Xvfb) screenshot: real honor-code text, real Solarized colors, a real live device code on screen. Wired into `SHANKPIT/apps/lobby` as a third launchable app. Real, named follow-up: PARENA editor font rendering (`stdlib/editor/render.prn`, same cgo-FFI pattern `internal/scrollmod/vterm_mod.prn` already proves) isn't wired in yet — this app still renders through PITVIPER's own existing Go font stack. See `IDUNA.GAME/NORTHSTAR.md`. |
| `EDITOR.GAME` | C (forked from PARENA) | New repo (2026-09-22) — founder real-time: "can we add the parena editor as a stand alone game in SHANKPIT_OS? EDITOR.GAME repo same affordances it currently has for saving a file." A real, standalone fork of `PARENA/examples/editor_main.c` + the real `stdlib/editor/*.prn` modules it drives (buffer, TextMate syntax highlighting, render, widget, split, spotlight) — same real save affordances PARENA's editor already has (F2, hover-reveal Save button, auto-format-on-save for `.prn` files), not reimplemented, just packaged as its own launchable SHANKPIT OS app. `gen/editor_stdlib_gen.c` is PARENA-generated, checked in as the real source of truth (same convention `PITVIPER/internal/scrollmod/vterm_mod.c` already established) — `make regenerate` needs a sibling `../PARENA` checkout. Verified for real: reproduced PARENA's own real `make editor-demo` build first (clean), then this fork's own `-Werror`-clean standalone build, then a real headless (Xvfb) run with a real `.prn` file as `argv[1]`, screenshotted showing genuine TextMate syntax-highlighted rendering, not a blank window. Wired into `SHANKPIT/apps/lobby` as a fifth launchable app. See `EDITOR.GAME/NORTHSTAR.md`. |
| `DEADWEIGHT_2` (being renamed to **D2**) | C (SDL2) | New repo (2026-09-25), upstream pre-created by the founder empty — founder real-time: "build DEADWEIGHT_2 (upstream repo created) — find the backpack battler plans in the founding deadweight docs." The backpack battler ("Dark Sector: Hold Battles" VS1 mechanic, `DEADWEIGHT/NORTHSTAR.md`) spun into its own repo since DEADWEIGHT itself shipped the unrelated card-mode VS0 game first (`DEADWEIGHT/docs/VS0_SCOPING.md`'s own build-order call). **Rename in progress** (founder real-time: "just call it D2, disambiguate it from DEADWEIGHT, D2 is the official studio name" — confirmed: rename the actual repo, not just branding): IDUNA's game slug/agent already renamed (`d2.play`/`d2.match.write`, `D2-SERVER` agent, `IDUNA@c5b16dc`); the GitHub repo itself is still named `DEADWEIGHT_2` (matching this row's own key) — **blocked**, the `GITHUB_TOKEN` in `EMILY/var/emily-secrets.env` shows `admin: true` in the repo's own permissions object but `PATCH .../DEADWEIGHT_2 {"name":"D2"}` is rejected ("Resource not accessible by personal access token") — needs either the founder renaming it directly or a token scoped for repo administration; local dir/remote-URL rename held until then so push/pull never breaks. Phase D1 (local-only core loop) and D2 (`dw2_server` — server-authoritative TCP 1v1, embedded FIFO matchmaking, no separate matchmaker binary; `dw2_client` — a real interactive client over the actual wire protocol, folded into D2 rather than a separate D3) both built and live-verified: 6x6 triple-bitmask grid, 7-item V0 catalog, energy routing (one-to-many via Splitter, many-to-one inert), Back-EMF closed-loop meltdown, live mid-fight Panic Cut, `tests/test_core_loop.c` (headless ASan+UBSan, 53 checks, hand-derived numbers), a real loadout beating an empty grid in a hand-derivable 21 ticks over the real wire protocol. CI (`.github/workflows/ci.yml`) generates a Principle 21 CONSTRUCT snapshot on every run, bundled into each client's own zip (SHANKPIT's own pattern) plus standalone `.zip`/`.gz`, collision-safe `build_<run>_<sha>`-tagged filenames. **Auto-release now live (SECTION 551, 2026-09-25)**: every green `main` push auto-bumps a MINOR version tag and publishes a real GitHub Release (`dw2_server`/`dw2_client`/`dw2_local` + `D2_CONSTRUCT.txt`, `_linux_x86_64`-suffixed) — live-verified, `v0.1.0` at `github.com/emilyspringerton/DEADWEIGHT_2/releases/tag/v0.1.0`, 4 real assets. Same job GPG-signs each binary and publishes to IDUNA's app-release registry (`app_slug="d2"`), matching DEADWEIGHT's own step — degrades to a clean no-op until its two GitHub Actions secrets are provisioned on this repo (verified: the CI log shows the expected skip message, not a silent failure), same current state as DEADWEIGHT. **Windows client added to releases (2026-09-25, founder real-time: "can we add windows client to deadweight 2 artifact releases")**: a new CI `windows` job cross-compiles `dw2_client.exe` via mingw (structurally verified — real `PE32+` binary, zero warnings under `-Werror` — but not runtime-tested on actual Windows, no Wine in this sandbox) and attaches `dw2_client_windows_x86_64.exe` + `SDL2.dll` to every release alongside the three Linux binaries; simpler than DEADWEIGHT's own Windows build since D2's `http.h` is plain-HTTP-only, no mbedTLS cross-build needed. Server/local stay Linux-only (scoped to the literal "client" ask). See `docs/WINDOWS_CLIENT_BUILD.md`. Real, named simplifications: a fixed hand-authored dummy opponent for local play (not a bot — D4's job), no real client art/EOSUI chrome yet, no PARENA integration yet (D5). Licensing deliberately left unresolved — explicit founder instruction not to add the Unlicense here, unlike `SKULDMARK`/`SPIDERBEETLE`'s own convention. See `NORTHSTAR.md`'s "Deferred" section for the remaining phased plan. |
...
## GOLDEN_DOCS Sync (standing instruction, monorepo-wide)

Founder real-time, 2026-09-21: `GOLDEN_DOCS` (`emilyspringerton/GOLDEN_DOCS`) is a full, real
snapshot of every golden doc (`EMILY/context/golden-docs-index.md`) plus core EMILY context
files. It has no source of its own — every file in it is a plain copy from this monorepo.

**Whenever you add, remove, or meaningfully edit a golden doc** — any file listed in
`EMILY/context/golden-docs-index.md`, or that file itself, or `EMILY/context/
full-system-context.md`/the sprawl memo/`golden-repos.json`/this file/`EMILY/docs/
ECOSYSTEM_CONTINUITY_REPORT.md` — **also resync `GOLDEN_DOCS` in the same unit of work**, same
spirit as README Reality above: the snapshot is a public claim about what the golden docs say,
and it must not lag behind them.

- **How:** run the regeneration script in `GOLDEN_DOCS/CLAUDE.md` (copies every file listed in
  `golden-docs-index.md` plus the context files, regenerates `MANIFEST.md`), then commit and push
  `GOLDEN_DOCS` — a separate commit in that repo, same as any other multi-repo change (see
  "Multi-repo changes" in The Emily Way above).
- **When it does not apply:** a change to a file that ISN'T a golden doc, even in a repo that has
  golden docs elsewhere; ordinary work that never touches `golden-docs-index.md` or a doc it
  lists.
- **Real automation is a known gap, not forgotten:** this manual resync is deliberate, not a
  placeholder for laziness — a real CI-triggered sync was considered and explicitly deferred
  (`EMILY/BACKLOG.md`) to avoid spending tokens on automation before the manual version has
  proven itself worth it. Do the manual resync; don't build the CI version unless asked.
