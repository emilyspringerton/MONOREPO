...
| `DEADWEIGHT_2` | C (SDL2 debug shell) | New repo (2026-09-25), upstream pre-created by the founder empty — founder real-time: "build DEADWEIGHT_2 (upstream repo created) — find the backpack battler plans in the founding deadweight docs." The backpack battler ("Dark Sector: Hold Battles" VS1 mechanic, `DEADWEIGHT/NORTHSTAR.md`) spun into its own repo since DEADWEIGHT itself shipped the unrelated card-mode VS0 game first (`DEADWEIGHT/docs/VS0_SCOPING.md`'s own build-order call). Phase D1 (local-only core loop, matching `DEADWEIGHT/docs/PHASE_D1_CORE_LOOP.md`'s exact scope) built and live-verified, not just scoped: the 6x6 triple-bitmask grid, a 7-item V0 catalog (Generator/Conductor/Splitter Node wiring pieces, one Railgun weapon, one Bulwark Plate armor piece, two splittable cargo items reproducing `docs/SPEC_REVIEW.md`'s own worked fragment-table examples verbatim), energy routing (one-to-many via Splitter, many-to-one inert), a Back-EMF closed-loop meltdown that permanently burns out the loop, and a live mid-fight Panic Cut. `tests/test_core_loop.c` (headless, ASan+UBSan, 53 checks) hand-derives and verifies exact numbers for every rule — a real 4-cell closed loop shatters on the exact predicted tick, one-to-many/many-to-one verified by exact charge counts, not just "no crash." `apps/local/main.c` is a bare-minimum SDL2 debug shell (pack a grid, fight a fixed dummy loadout) with a headless `--selftest` that runs a full scripted match through the real render path. Real, named simplifications in `NORTHSTAR.md`: one combined Back-EMF loop tracker (not independently-many), a fixed hand-authored dummy opponent (not a real bot — that's D2's job once a server exists), no drag-and-drop/art/font renderer yet. Licensing deliberately left unresolved — explicit founder instruction not to add the Unlicense here, unlike `SKULDMARK`/`SPIDERBEETLE`'s own convention. No networking/accounts/PARENA integration yet, all named as real future phases (D2-D5) in `NORTHSTAR.md`. |
