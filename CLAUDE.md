...
| `OpenExecutive` | Python (FastAPI) + Next.js 15 | Forked 2026-09-20 from [SenteLabsAI/OpenExecutive](https://github.com/SenteLabsAI/OpenExecutive) (real, credited prior art, Apache 2.0, 155+ merged upstream PRs of history) — a multi-agent "virtual corporate executive" (CSO/CFO/CHRO/GC/COO/CMO/CPO/Board specialists behind one coherent persona), Anthropic Claude backbone, ChromaDB RAG, SQLite episodic memory, Slack/email/Telegram/Discord/Google Chat/MCP integrations. This monorepo's real, in-progress work: wiring identity to **IDUNA** (M2M provisioning, `POST /api/v1/openexecutive/provision`, real and live in IDUNA `main.go`) and migrating the LLM backbone to **Google Vertex AI (Gemini)**. S506 NORTHSTAR pass (2026-09-21) found the Gemini/IDUNA-auth code Claude Haiku 4.5 wrote has two confirmed, concrete bugs (wrong JWT key algorithm — EC vs RSA; wrong tool-schema shape — Anthropic vs OpenAI) and is not wired into config/the provider registry/any route yet; repo relocated from a `/tmp` clone to here. See `OpenExecutive/NORTHSTAR.md` for the full critical review and phased next steps. |
