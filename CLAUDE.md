...
| `skateboard` (project name **Papercraft**) | C/SDL2 (planned) | Pre-existing repo ("SkateChain: The Polyglot Metaverse," the ancestor `SHANKPIT`/REDGARDEN/GFD all descended from — `apps/shank-fps` is SHANKPIT's own direct ancestor) — NORTHSTAR-only, named 2026-08-28 (founder: "papercraft... thats the name of the thing we are building on SKATE skate culture northstar" / "instead of minecraft we iterate shankpit into papercraft" / "not voxels"). GTA3 × Skate 2 × R6-Siege-style **non-voxel** destructible city, set in the TRAPX universe as a sibling product (not a replacement) to GFD's own voxel-based TRAPX — built by iterating SHANKPIT's own lineage forward, explicitly not the GoblinFoxDragon/DragonsNShit voxel engine. See `skateboard/NORTHSTAR.md` for the full scoping; not started as actual code yet. |
